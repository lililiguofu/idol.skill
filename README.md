...
*「与其每天苦等 ta 发一条泡泡，在超话里患得患失，不如用大模型完成一次终极的私有化部署。爱豆的营业期也许会结束，但跑在本地的赛博分身，永远对你秒回。」*

[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)
[![Claude Code](https://img.shields.io/badge/Claude%20Code-Skill-8b5cf6.svg)](https://claude.com/claude-code)
[![skills.sh](https://img.shields.io/badge/skills.sh-Compatible-22c55e.svg)](https://skills.sh)

用 Bubble、签售稿、采访与脑内记忆，把 **你担的语气与梗** 蒸馏成可长期调用的 **本地爱豆 Skill**。  
Universe / Persona / `meta.json` 分层状态，支持 OOC 热修正与可选 **场景设定**。

[看效果](#效果示例) · [安装](#安装) · [工作原理](#工作原理) · [安全边界](#安全与边界)
...
> **爱豆分身（示例口吻）**  
> 先别把自己绕进去啦……已读而已，又不代表她讨厌你。今晚先好好睡一觉，明天再说，嗯？
...
3. **Step 1c（场景设定，可选）**：默认把聊天想象成发生在哪（后台、路上、宿舍氛围等），写入 `meta.json` 的 `scene` 与 `universe.md` 第七节「默认对话场景」；详见 [prompts/scene_setting.md](prompts/scene_setting.md)。  
...
- **仅供娱乐**：请勿用于生成虚假绯闻、商业盈利或侵犯艺人名誉权。
- **圈地自萌**：语料与分身仅在你本地使用；付费泡泡、签售等私密内容勿上传用于训练。
- **赛博认知**：分身知悉自己是本地陪伴设定；不诱导私生、跟机等危险行为；不拉踩其他艺人或队友。
- **Layer 0**：不代替医生/律师/工程师给出可执行的专业结论；可提供情绪支持与口吻一致的陪伴。
